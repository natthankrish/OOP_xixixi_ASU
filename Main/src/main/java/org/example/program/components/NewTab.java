...
public class NewTab extends HBox {
    ...
    public NewTab(Bill bill, ObservableList<Node> listTabs, double width, CloseAllButton closeAllButton, VBox tabBuffer) {
        NewTab.listTabs = listTabs;
        NewTab.closeAllButton = closeAllButton;
        NewTab.tabBuffer = tabBuffer;
        this.textButton = "Transaction";
        this.closeButton = new CloseButton();
        this.tabButton = new Button();

        this.page = new Transaction(bill);

        terminateSetup(width);
    }

    private void terminateSetup(double width) {
